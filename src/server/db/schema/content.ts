...
/**
 * Detail tables follow one shape: a foreign key into the entity registry, typed
 * columns for the facets that drive filtering and sorting, and `data` holding
 * the complete original object.
 *
 * Typed columns exist for querying, not display — the renderer always reads
 * `data`. To filter on a facet not projected here, add a column and re-ingest.
 */
const entityRef = () =>
  uuid()
    .primaryKey()
...
export const spells = pgTable(
  "spells",
  {
    ...
    /**
     * Classes able to cast this spell, Title-cased. Derived from
     * `generated/gendata-spell-source-lookup.json` rather than the spell object
     * — the mapping is stored externally upstream.
     *
     * Merges the lookup's `class` and `classVariant` grants. `classVariant`
     * records a later book adding a spell to an earlier class's list, and is
     * the only grant 119 spells have; reading `class` alone leaves every XGE
     * spell invisible to the class filter.
     *
     * Empty is meaningful: 16 spells are subclass-only and belong to no class
     * list.
     */
    classes: text().array(),
...
export const monsters = pgTable(
  "monsters",
  {
     ...
     * fractional CRs become 0.125/0.25/0.5. Null where there is no usable CR
     * (some templates and NPC stat blocks).
     */
    ...
    /** Upstream `traitTags` + `actionTags`, flattened for facet filtering. */
    tags: text().array(),
...
export const items = pgTable(
  "items",
  {
    ...
    /** Upstream type abbreviation: "M" melee, "HA" heavy armor, "SCF"... */
     ...
     * to a base item (e.g. "+1 Longsword"). Upstream stores these as templates
     * and expects the consumer to expand them.
     */
...
export const classes = pgTable("classes", {
  ...
  /** Prepared vs known casting; the character builder branches on this. */
 ...
 * pact boons — anything modelled upstream as a class-granted option list.
 */
 ...
 * traps, vehicles, objects, rewards, psionics, tables and the like. These are
 * browsed and cross-referenced but never filtered on type-specific facets, so
 * they share one table. Promote a type out of here when it needs filtering.
 */
